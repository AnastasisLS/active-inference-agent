...
class DQNAgent(BaseAgent):
    ...
    def get_action_distribution(self, state: np.ndarray) -> np.ndarray:
        ...
        Returns:
            Action probabilities (softmax of Q-values)
        """
        with torch.no_grad():
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to(self.device)
            q_values = self.q_network(state_tensor)
    ...
    def get_value_estimate(self, state: np.ndarray) -> float:
        ...
        Returns:
            Maximum Q-value (value estimate)
        """
        with torch.no_grad():
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to(self.device)
            q_values = self.q_network(state_tensor)
...
# Convenience function for creating DQN agent
def create_dqn_agent(
    ...
    """
    Create a DQN agent with specified parameters.
